    ...
    public function students(Request $request, $is_export_excel = false)
{
    ...
    // Allow triggering export via query/body: ?export_excel=1
    $is_export_excel = $is_export_excel || $request->boolean('export_excel');

    // Base query: only students
    $query = User::where('role_name', Role::$user);

    // High-level counts (unfiltered)
    ...
    // Existing filters (your app-specific filter method)
    $query = $this->filters($query, $request);

    // --- New field filters (all optional) ---
    ...
    $query->when($request->filled('nationality'), function ($q) use ($request) {
        $q->where('nationality', 'like', '%' . trim($request->input('nationality')) . '%');
    });

    $query->when($request->filled('address'), function ($q) use ($request) {
        $q->where('address', 'like', '%' . trim($request->input('address')) . '%');
    });

    // birthdate range: birthdate_from=YYYY-MM-DD&birthdate_to=YYYY-MM-DD
    $query->when($request->filled('birthdate_from'), function ($q) use ($request) {
        $q->whereDate('birthdate', '>=', $request->input('birthdate_from'));
    });
    $query->when($request->filled('birthdate_to'), function ($q) use ($request) {
        $q->whereDate('birthdate', '<=', $request->input('birthdate_to'));
    });

    // has_images: 1 = require both images, 0 = require at least one missing
    $query->when($request->filled('has_images'), function ($q) use ($request) {
        if ((string)$request->input('has_images') === '1') {
            $q->whereNotNull('user_image')->where('user_image', '!=', '')
              ->whereNotNull('nid_image')->where('nid_image', '!=', '');
        } elseif ((string)$request->input('has_images') === '0') {
            $q->where(function ($qq) {
                $qq->whereNull('user_image')->orWhere('user_image', '=', '')
                   ->orWhereNull('nid_image')->orWhere('nid_image', '=', '');
            });
        }
    });

    // Optional: parents info contains (fathers_name/mothers_name/email/phone)
    $query->when($request->filled('parent_q'), function ($q) use ($request) {
        $needle = '%' . trim($request->input('parent_q')) . '%';
        $q->where(function ($qq) use ($needle) {
            $qq->where('fathers_name', 'like', $needle)
               ->orWhere('fathers_email', 'like', $needle)
               ->orWhere('fathers_contact_number', 'like', $needle)
               ->orWhere('mothers_name', 'like', $needle)
               ->orWhere('mothers_email', 'like', $needle)
               ->orWhere('mothers_contact_number', 'like', $needle);
        });
    });

    // Sort newest first unless a sort is provided
    ...
    // Fetch
    $users = $is_export_excel
        ? $query->get()
        : $query->paginate(10)->appends($request->query());

    // Enrich user rows with any extra info you already compute
    $users = $this->addUsersExtraInfo($users);

    if ($is_export_excel) {
        // Return the collection for your Excel exporter to handle upstream
        return $users;
    }
    ...
    $data = [
        ...
        // pass current filters back to the view if helpful
        'filters' => $request->only([
            'sex','nationality','address','birthdate_from','birthdate_to','has_images','parent_q','sort_by','sort_dir'
        ]),
    ];
    ...
    public function store(Request $request)
{
    ...
    // Figure out which field is the "username" (email or mobile in your app)
    $username = $this->username($data);
    $data[$username] = $data['username'] ?? null;
    $request->merge([$username => $data[$username]]);
    unset($data['username']);

    // Validation (idempotent + supports file or string for images)
    $this->validate($request, [
        $username   => ($username === 'mobile')
            ? 'required|numeric|unique:users,'.$username
            : 'required|string|email|max:255|unique:users,'.$username,
        ...
        // Accept either a real file upload OR a string path from an external file manager
        'user_image' => $request->hasFile('user_image')
            ? 'image|mimes:jpeg,png,jpg,webp|max:2048'
            : 'nullable|string|max:255',
        'nid_image'  => $request->hasFile('nid_image')
            ? 'image|mimes:jpeg,png,jpg,webp|max:4096'
            : 'nullable|string|max:255',
    ]);
    ...
    if (!empty($data['role_id'])) {
        ...
        if (!empty($role)) {
            ...
            // Handle file uploads if present (store public URL like /storage/...)
            if ($request->hasFile('user_image')) {
                $path = $request->file('user_image')->store('users', 'public');
                $data['user_image'] = '/storage/'.$path;
            }
            if ($request->hasFile('nid_image')) {
                $path = $request->file('nid_image')->store('nid', 'public');
                $data['nid_image'] = '/storage/'.$path;
            }
            ...
            // Optional: assign to group and notify
    ...
    public function edit(Request $request, $id)
{
    ...
    $user = User::where('id', $id)
        ->with([
            'customBadges' => function ($query) { $query->with('badge'); },
            'occupations' => function ($query) { $query->with('category'); },
            'organization' => function ($query) { $query->select('id', 'full_name'); },
    ...
    $provinces = $cities = $districts = null;

    $countries = Region::select(DB::raw('*, ST_AsText(geo_center) as geo_center'))
        ->where('type', Region::$country)->get();

    if (!empty($user->country_id)) {
        $provinces = Region::select(DB::raw('*, ST_AsText(geo_center) as geo_center'))
            ->where('type', Region::$province)->where('country_id', $user->country_id)->get();
    }

    if (!empty($user->province_id)) {
        $cities = Region::select(DB::raw('*, ST_AsText(geo_center) as geo_center'))
            ->where('type', Region::$city)->where('province_id', $user->province_id)->get();
    }

    if (!empty($user->city_id)) {
        $districts = Region::select(DB::raw('*, ST_AsText(geo_center) as geo_center'))
            ->where('type', Region::$district)->where('city_id', $user->city_id)->get();
    }

    $userBanks = UserBank::query()->with(['specifications'])->orderBy('created_at', 'desc')->get();

    $userType = $user->isTeacher() ? "teacher" : ($user->isUser() ? "user" : "organization");
    ...
    $data = [
        ...
        // extras for your view
        'sexOptions' => ['male' => 'Male', 'female' => 'Female', 'other' => 'Other'],
    ];
    ...
    if (auth()->user()->can('admin_forum_topics_lists')) {
        $data['topics'] = ForumTopic::where('creator_id', $user->id)
            ->with([
                'posts' => function ($query) { $query->orderBy('created_at', 'desc'); },
                'forum'
            ])
            ->withCount('posts')
            ->orderBy('created_at', 'desc')
